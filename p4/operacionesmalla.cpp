...
void VerISPlano( MallaTVT *malla ){ //usando normales de caras
   glEnable(GL_LIGHTING);
   glEnable(GL_NORMALIZE);
   glShadeModel (GL_FLAT);
   glBegin(GL_TRIANGLES);
   for (int i=0; i<malla->caras.size(); i++){ 
      if (malla->cte_ver.size() != 0){  //cambio la coordenada de textura
   	 glTexCoord2f(malla->cte_ver[(malla->caras[i].idx[0])].coo[0], malla->cte_ver[(malla->caras[i].idx[0])].coo[1]);
      }
      glNormal3f(malla->normales_car[2*(i) +1].coo[0] - malla->normales_car[2*(i)].coo[0],
                 malla->normales_car[2*(i) +1].coo[1] - malla->normales_car[2*(i)].coo[1],
                 malla->normales_car[2*(i) +1].coo[2] - malla->normales_car[2*(i)].coo[2]);
      glVertex3f(malla->vertices[malla->caras[i].idx[0]].coo[X],
                 malla->vertices[malla->caras[i].idx[0]].coo[Y],
                 malla->vertices[malla->caras[i].idx[0]].coo[Z]);

      if (malla->cte_ver.size() != 0){  //cambio la coordenada de textura
         glTexCoord2f(malla->cte_ver[(malla->caras[i].idx[1])].coo[0], malla->cte_ver[(malla->caras[i].idx[1])].coo[1]);
      }
      glNormal3f(malla->normales_car[2*(i) +1].coo[0] - malla->normales_car[2*(i)].coo[0],
                 malla->normales_car[2*(i) +1].coo[1] - malla->normales_car[2*(i)].coo[1],
                 malla->normales_car[2*(i) +1].coo[2] - malla->normales_car[2*(i)].coo[2]);
      glVertex3f(malla->vertices[malla->caras[i].idx[1]].coo[X],
                 malla->vertices[malla->caras[i].idx[1]].coo[Y],
                 malla->vertices[malla->caras[i].idx[1]].coo[Z]);

      if (malla->cte_ver.size() != 0){  //cambio la coordenada de textura
         glTexCoord2f(malla->cte_ver[(malla->caras[i].idx[2])].coo[0], malla->cte_ver[(malla->caras[i].idx[2])].coo[1]);
      }
      glNormal3f(malla->normales_car[2*(i) +1].coo[0] - malla->normales_car[2*(i)].coo[0],
                 malla->normales_car[2*(i) +1].coo[1] - malla->normales_car[2*(i)].coo[1],
                 malla->normales_car[2*(i) +1].coo[2] - malla->normales_car[2*(i)].coo[2]);
      glVertex3f(malla->vertices[malla->caras[i].idx[2]].coo[X],
                 malla->vertices[malla->caras[i].idx[2]].coo[Y],
                 malla->vertices[malla->caras[i].idx[2]].coo[Z]);
   }
   glEnd();
}
void VerISSuave( MallaTVT *malla ){
   glEnable(GL_LIGHTING);
   glEnable(GL_NORMALIZE);
   glShadeModel (GL_SMOOTH);
   glBegin(GL_TRIANGLES);
   for (int i=0; i<malla->caras.size(); i++){ 
      if (malla->cte_ver.size() != 0){  //indico la coordenada de textura
   	 glTexCoord2f(malla->cte_ver[(malla->caras[i].idx[0])].coo[0], malla->cte_ver[(malla->caras[i].idx[0])].coo[1]);
      }
      glNormal3f(malla->normales_ver[2*(malla->caras[i].idx[0]) +1].coo[0] - malla->normales_ver[2*(malla->caras[i].idx[0])].coo[0],
                 malla->normales_ver[2*(malla->caras[i].idx[0]) +1].coo[1] - malla->normales_ver[2*(malla->caras[i].idx[0])].coo[1],
                 malla->normales_ver[2*(malla->caras[i].idx[0]) +1].coo[2] - malla->normales_ver[2*(malla->caras[i].idx[0])].coo[2]);
      glVertex3f(malla->vertices[malla->caras[i].idx[0]].coo[X],
                 malla->vertices[malla->caras[i].idx[0]].coo[Y],
                 malla->vertices[malla->caras[i].idx[0]].coo[Z]);
      ...
      glNormal3f(malla->normales_ver[2*(malla->caras[i].idx[1]) +1].coo[0] - malla->normales_ver[2*(malla->caras[i].idx[1])].coo[0],
                 malla->normales_ver[2*(malla->caras[i].idx[1]) +1].coo[1] - malla->normales_ver[2*(malla->caras[i].idx[1])].coo[1],
                 malla->normales_ver[2*(malla->caras[i].idx[1]) +1].coo[2] - malla->normales_ver[2*(malla->caras[i].idx[1])].coo[2]);
      glVertex3f(malla->vertices[malla->caras[i].idx[1]].coo[X],
                 malla->vertices[malla->caras[i].idx[1]].coo[Y],
                 malla->vertices[malla->caras[i].idx[1]].coo[Z]);
      ...
      glNormal3f(malla->normales_ver[2*(malla->caras[i].idx[2]) +1].coo[0] - malla->normales_ver[2*(malla->caras[i].idx[2])].coo[0],
                 malla->normales_ver[2*(malla->caras[i].idx[2]) +1].coo[1] - malla->normales_ver[2*(malla->caras[i].idx[2])].coo[1],
                 malla->normales_ver[2*(malla->caras[i].idx[2]) +1].coo[2] - malla->normales_ver[2*(malla->caras[i].idx[2])].coo[2]);
      glVertex3f(malla->vertices[malla->caras[i].idx[2]].coo[X],
                 malla->vertices[malla->caras[i].idx[2]].coo[Y],
                 malla->vertices[malla->caras[i].idx[2]].coo[Z]);
   }
...
//********************************************************************************************
void CrearObjetoRevolucion2(MallaTVT *malla , int numero_perfiles, bool tapas){
//si el primero están en el eje lo quitamos de los vértices (posteriormente se añade) 
   if(malla->vertices_ply[0] == 0.0){ 
     for (int i=0; i< malla->vertices_ply.size()-3; i++){
       malla->vertices_ply[i] = malla->vertices_ply[i+3];
   ...
   //crear perfiles del sólido
   double n_perfiles=numero_perfiles; //leer el numero de perfiles a realizar
   double angulo = 0.0;  //calculamos el ángulo de rotación 
   //obtenemos el numero de vertices de un perfil
   malla->num_vertices = malla->vertices_ply.size();
   double n_v_perfil=(malla->num_vertices)/3;
   for (int j=0; j< n_perfiles; j++){
       ...
       angulo=(j+1)*((2*PI)/(n_perfiles-1));
   }
   malla->num_vertices=(malla->num_vertices)*(n_perfiles);


//hemos modificado la creacion de las caras con respecto al crearsolidorevolucion
   //crear caras del sólido.
   malla->num_caras=(3*((n_v_perfil-1)*(n_perfiles)))*2;
   Tupla3i carad;
   Tupla3i carai;
   int perfils=n_perfiles;
     for(int k=0; k < (perfils) ; k++){ 
       for(int i = 0; i < ( n_v_perfil-1) ; i++){  //caras formadas por dos perfiles del sólido
         //cara derecha
         carad.idx[0]=i+((k+1)%(perfils)*n_v_perfil);
         carad.idx[1]=i+(k*n_v_perfil)+1;
         carad.idx[2]=i+(k*n_v_perfil);
         malla->caras.push_back(carad);
         //cara izquierda
         carai.idx[0]=i+((k+1)%(perfils)*n_v_perfil);
         carai.idx[1]=i+1 + n_v_perfil*((k+1)%(perfils));
         carai.idx[2]=i+1+(k*n_v_perfil);
         malla->caras.push_back(carai);
       }
      }
if(tapas){
   //Creamos la tapa inferior
      ...
      //añadimos las caras de la tapa inferior   
      Tupla3i cara;
      malla->num_caras= malla->num_caras+ ((n_perfiles-1)*3);
      for(int i=0; i< ( perfils); i++){
         cara.idx[0]=(perfils*n_v_perfil);
         cara.idx[1]=(i*n_v_perfil);
         cara.idx[2]=((i+1)%perfils)*n_v_perfil;
         malla->caras.push_back(cara);
      }
      ...
      malla->num_caras= malla->num_caras+ ((n_perfiles-1)*3);
      for(int i=0; i< ( perfils ); i++){
         cara.idx[0]=(perfils*n_v_perfil) +1; 
         cara.idx[1]=(i*n_v_perfil) + n_v_perfil -1;
         cara.idx[2]=((i+1)%perfils)*n_v_perfil + n_v_perfil -1;
         malla->caras.push_back(cara);
      }
}

     //creacion de la tabla de coordenadas de textura.
     float divisors = n_perfiles-1 ;
     float coord_t=0.0;
     float coord_s=0.0;
     std::vector<float> vectord;
     vectord.push_back(0.0);
     float diferencia=0.0;
     for(int k=0; k<n_v_perfil ; k++){
        diferencia= sqrt((pow(malla->vertices[k+1].coo[0] - malla->vertices[k].coo[0],2.0)) + 
                         (pow(malla->vertices[k+1].coo[1] - malla->vertices[k].coo[1],2.0)) + 
                         (pow(malla->vertices[k+1].coo[2] - malla->vertices[k].coo[2],2.0)));
        vectord.push_back(vectord.back() + diferencia);
     }
     for (int i=0; i<n_perfiles; i++){
        coord_s=i/divisors;
        for(int j=0; j<n_v_perfil;j++){
        coord_t = vectord[j] / vectord[n_v_perfil-1];
        Tupla2f coord_v(coord_s,1-coord_t); 
        malla->cte_ver.push_back(coord_v);
        }
     }
